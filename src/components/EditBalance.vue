...
export default {
  ...
  methods: {
    ...mapActions(["EditBalanceGeneral", "getBalanceGeneral"]),
    async save() {
      let balance = { ...this.doc };
      balance.anio = this.doc.anio;

      let totales = {
        activos:
          parseFloat(balance.activos.caja_y_bancos) +
          parseFloat(balance.activos.cartera_de_prestamos) +
          parseFloat(balance.activos.inversiones_financieras) +
          parseFloat(balance.activos.operaciones_bursatiles) +
          parseFloat(balance.otros_activos.diversos) +
          parseFloat(balance.activos_fijos.bienes_inmuebles),

        pasivos:
          parseFloat(balance.pasivos.prestamos_del_banco) +
          parseFloat(balance.pasivos.prestamo_bancos) +
          parseFloat(balance.pasivos.titulos_de_emision_propia) +
          parseFloat(balance.pasivos.fondos_de_administracion) +
          parseFloat(balance.pasivos.provisiones) +
          parseFloat(balance.pasivos.diversos),

        patrimonio:
          parseFloat(balance.patrimonio.aportes_del_estado) +
          parseFloat(balance.patrimonio.reservas_del_capital),
        pasivo_y_patrimonio:
          parseFloat(balance.pasivos.prestamos_del_banco) +
          parseFloat(balance.pasivos.prestamo_bancos) +
          parseFloat(balance.pasivos.titulos_de_emision_propia) +
      ...
      balance.total_activos = totales.activos.toFixed(1);
      balance.total_pasivos = totales.pasivos;
      balance.total_pasivos_patrimonio = totales.pasivo_y_patrimonio.toFixed(1);
      ...
      if (balance.anio != "") {
        let guardo = await this.EditBalanceGeneral(balance);
        if (guardo) {
